...
The goal of [SPD](https://github.com/orca-scan/simplified-product-data) is to provide the above information to the developer in the first request. Additional information such as ingredients can be obtained by following the related HATEOAS link `rel=gs1:ingredientsInfo` in the `links` array _(if present)_.
